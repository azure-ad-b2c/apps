...
Navigate to the `"apps/web-dotnet-core-b2c-embedded"` folder

 ```Sh
  cd "apps/web-dotnet-core-b2c-embedded"
  ```
...
To use Azure AD B2C Embedded sign-in, you need to first enable a custom domain with your Azure AD B2C account, and configure it with a custom domain which is on the same domain as your published application, i.e. **Same Origin**. This is required because Azure AD B2C should not allow you to load the sign-in experience in an iframe unless you enable CORS for the source domains loading the iframe using a custom policy. So first, enable a custom domain similar to `login.contoso.com` if your application is published on something similar to `www.contoso.com`. [Tutorial: Enable custom domains for Azure Active Directory B2C](https://docs.microsoft.com/en-us/azure/active-directory-b2c/custom-domain?pivots=b2c-custom-policy). I've also created a video tutorial [here](https://youtu.be/-J_FSQsZxmM).
...
<RelyingParty>
  ...
  <UserJourneyBehaviors> 
    <JourneyFraming Enabled="true" Sources="https://contoso.com https://www.contoso.com" /> 
  </UserJourneyBehaviors>
<!--
</RelyingParty> -->
```
...
```xml
PublicPolicyUri="http://login.contoso.com/B2C_1A_signup_signin"
```
...
- Set the **Reply URL** to `https://www.contoso.com/signin-oidc`.
...
1. Find the assignment for `Instance` and replace the value with your tenant name. For example, `https://login.contoso.com`
1. Find the assignment for `Domain` and replace the value with your Azure AD B2C domain name. For example, `contoso.onmicrosoft.com`
...
1. Find the assignment for `ResetPasswordPolicyId` and replace with the name of the `password reset` policy you created in Step 4.
1. Find the assignment for `EditProfilePolicyId` and replace with the name of the `profile edit` policy you created in Step 4.
...
```JSon
{
  "AzureAdB2C": {
    ...
    "SignedOutCallbackPath": "/signout/B2C_1A_signup_signin_embedded",
    "SignUpSignInPolicyId": "B2C_1A_signup_signin_embedded",
    "ResetPasswordPolicyId": "B2C_1A_PasswordReset_embdedded",
    "EditProfilePolicyId": "B2C_1A_ProfileEdit_embedded"
  }
}
```
