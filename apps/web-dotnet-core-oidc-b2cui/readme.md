...
Learn how to restrict access to your [.NET Core MVC web application](https://docs.microsoft.com/aspnet/core/mvc/overview?) to clients that have authenticated with Azure Active Directory B2C (Azure AD B2C). This sample covers calling an OpenID Connect identity provider (Azure AD B2C) using [OpenID Connect](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-reference-oidc).

This code in this article is based on the [An ASP.NET Core Web app signing-in users with the Microsoft identity platform in Azure AD B2C](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/tree/master/1-WebApp-OIDC/1-5-B2C) sample.

## How the sample app generated by this guide works

![OIDC flow](https://docs.microsoft.com/azure/active-directory/develop/media/active-directory-develop-guidedsetup-aspnetwebapp-intro/aspnetbrowsergeneral.svg)
...
* [User flows](tutorial-create-user-flows.md) or [custom policies](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-get-started-custom?tabs=applications) created in your tenant

## 1. Setup your web application

This section describes how to install and configure the authentication pipeline with the ASP.NET Core `Authentication.AzureADB2C.UI` middleware on an MVC project by using OpenID Connect.
...
The middleware is initialized in the `Startup.cs` file by passing the default authentication scheme `AzureADB2COptions`. The options are read from the `appsettings.json` file.

The middleware takes care of:

- Requesting OpenID Connect sign-in using the policy from the `appsettings.json` file.
- Processing OpenID Connect sign-in responses by validating the signature and issuer in an incoming ID token (a JWT), extracting the user's claims, and putting the claims in `ClaimsPrincipal.Current`.
- Integrating with the session cookie ASP.NET Core middleware to establish a session for the user.

You can trigger the middleware to send an OpenID Connect sign-in request by decorating a class or method with the `[Authorize]` attribute or by issuing a challenge. For more details, see the [AccountController.cs](https://github.com/aspnet/AspNetCore/blob/master/src/Azure/AzureAD/Authentication.AzureADB2C.UI/src/Areas/AzureADB2C/Controllers/AccountController.cs) source file, part of ASP.NET Core.


1. Open the **Startup.cs**

1. Add the following using directives to Startup.cs:
...
1. Locate the `public void ConfigureServices(IServiceCollection services)` method and add the following code snippet just before the closing of the method:

    ```csharp
    // Handling SameSite cookie according to https://docs.microsoft.com/aspnet/core/security/samesite?view=aspnetcore-3.1
...
1. Locate the `Configure(IApplicationBuilder app, IWebHostEnvironment env)` method and add the following code snippet just before the line with `app.UseAuthorization();`:
...
1. Open the `_LoginPartial.cshtml` file and add following code snippet
    ...
    @if (User.Identity.IsAuthenticated)
    {
        <ul class="nav navbar-nav navbar-right">
            ...
            <!-- The Account controller is not defined in this project. Instead, it's part of the
                 Authentication.AzureADB2C.UI NuGet package, and it defines some well-known actions like
                 SignIn, SignOut and EditProfile. -->
            <li class="navbar-btn">
...
Open your **appsettings.json** file and add following entries at the top level of the JSON. Replace:
...
The above configuration requires that you run the application at the `https://localhost:44316/` URL. If you run it on a different URL or port, edit the `Properties/launchSettings.json` file and specify the new URL. For example:
